...
-- Keep notification data outside Supabase's API-facing `public` schema. The
-- service connects directly to Postgres and sets this schema on every session.
create schema if not exists smokeshow_notify;
set search_path = smokeshow_notify, public;
...
create table if not exists devices (
  ...
  notify_inbound       boolean     not null default true,
  notify_peak          boolean     not null default true,
  notify_clear         boolean     not null default true,
...
-- Keep this migration idempotent so applying schema.sql is still the one-step
-- deployment documented in the service README.
alter table devices add column if not exists notify_inbound boolean not null default true;
alter table devices add column if not exists notify_peak boolean not null default true;
alter table devices add column if not exists notify_clear boolean not null default true;
