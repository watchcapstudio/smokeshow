...
and `src/data/sources.js` (data credits). `npm run pages` writes them; `npm run
build` runs it first. They are static HTML because the whole point is that a
crawler sees the city's words in the initial payload.
