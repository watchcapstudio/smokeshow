...
4. Map: Leaflet + canvas smoke layer (pale smoke on a dark basemap, not AQI colors), scrubber -12hr/+48hr, night shading, 5-day strip
   ...
   - Active fire layer: NASA FIRMS heat detections, clustered by `scripts/hrrr/fetch_fires.py` in the same 4×/day Actions job that renders the HRRR frames, published as `fires.json` on the `data` branch. Needs a `FIRMS_MAP_KEY` repo secret (free, by email from NASA) — without it the step no-ops and the layer is simply absent, which is a supported state. They are thermal hotspots, not confirmed fires and not named incidents: never label them otherwise. `npm run verify:fires` captures the layer over both backdrops it has to survive and fails if the icon stops clearing 3:1 against either.
