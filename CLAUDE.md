...
4. Map: Leaflet + canvas smoke layer (pale smoke on a dark basemap, not AQI colors), scrubber -12hr/+48hr, night shading, 5-day strip
   ...
   - `SMOKE_STOPS` in `src/lib/rating.js` is hand-mirrored in `scripts/render/ramp.py` — those are the only two copies, and both renderers import the Python one. Change both, then run `npm run ramp` — it proves the composite stays monotonic, that the 256-entry PNG-8 palette the frames actually carry is that ramp, and fails if the copies drift.
