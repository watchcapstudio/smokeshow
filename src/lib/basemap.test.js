...
describe('tile URLs', () => {
  // The bug this guards is silent: hardcode a theme here and the tiles keep
  // working while the ramp is audited against the other one, so the plume
  // converges with the land instead of standing off it.
  it('derive their theme from BASEMAP_THEME', () => {
    expect(CARTO_BASE_URL).toContain(`${BASEMAP_THEME}_nolabels`);
    expect(CARTO_LABELS_URL).toContain(`${BASEMAP_THEME}_only_labels`);
  });
});
